...
public class TabSmoothView extends View {
	...
	public TabSmoothView(Context context, AttributeSet attrs, int defStyle)  {
		...
		mTabs=new ArrayList<TabSmoothView.TabItem>();
		...
		if(items!=null){
			for (CharSequence item : items) {
				mTabs.add(new TabItem(item.toString()));
			}
		}
	}
	...
	public void addTab(String text){
		addTab(new TabItem(text));
	}
	public void addTab(TabItem item){
		mTabs.add(item);
		requestLayout();
	}
	public void addTabs(List<TabItem> tabs){
		if(tabs==null){
			return;
		}
		mTabs.addAll(tabs);
...
//				}
	...
	public static interface OnTabItemClickListener{
		void onTabItemClick(int postion,Object item);
	}
